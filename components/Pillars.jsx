...
// Maps the accent key from content.js to top-border + tag colors.
const accentMap = {
      ...
      <div className="shell">
        ...
        <div className="mt-10 grid gap-5 sm:grid-cols-2 lg:grid-cols-4">
          {pillars.items.map((item) => (
            <article
              key={item.tag}
              className={`rounded-card border-[1.5px] border-border1 border-t-4 bg-white p-6 dark:border-night-border dark:bg-night-card ${accentMap[item.accent]}`}
            >
              <span
